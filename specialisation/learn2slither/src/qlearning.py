...
class QLearningAgent:
	def __init__(self, epsilon=0.2, alpha=0.2, num_training_episodes=100000):
		...
		self.alpha = alpha
		self.q_values = {}
		self.discount_factor = 1  # Note: This variable is not used in the code.
	...
	def copy(self):
		...
		new_agent = QLearningAgent(self.epsilon, self.alpha, self.num_training_episodes)
		new_agent.q_values = copy.deepcopy(self.q_values)
		return new_agent

	def get_state(self, game):
		# Get a unique representation of the game state
		# The snake knows only what is in front of him, behind him, right and left
		head = game.snake[0]

		is_front_blocked = self.is_wall_or_body(game, head[0], head[1], 'up')
		is_left_blocked = self.is_wall_or_body(game, head[0], head[1], 'left')
		is_right_blocked = self.is_wall_or_body(game, head[0], head[1], 'right')
		is_behind_blocked = self.is_wall_or_body(game, head[0], head[1], 'down')
	...
	def get_reward(self, game, action):
		...
		# Create a dictionary for distances to each visible green apple on the same row or column
	...
	def choose_action(self, state):
		...
		if self.is_learning and random.uniform(0, 1) < self.epsilon:
			...
			best_actions = [
				a for a, q in zip(possible_actions, q_values) if q == max_q_value
			]
