...
class SnakeGameGUI:
    ...
    def run(
        ...
        train=False,
        num_episodes=1000,
        model_path="model/q_values_1000.json",
        want_visual=False,
        save_model=None,
        step_by_step=False,
    ):
        ...
        if not train:
            while not self.game.is_game_over and not self.game.is_won:
                time.sleep(0.1)
            ...
            if save_model:
                self.game.agent.save_q_values(
                    filename=save_model, is_final=True
                )
            else:
                self.game.agent.save_q_values(
                    filename=f"q_values_{num_episodes}.json", is_final=True
                )
