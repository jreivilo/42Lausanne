...
class SnakeGame:
    ...
    def initialize_snake(self):
        # Randomly choose a starting point for the snake, ensuring it fits on
        # the board
    ...
    def place_apples(self):
        ...
        while len(self.green_apples) < 2:
            ...
            if (
                ...
                and position not in self.green_apples
            ):
        ...
        while (
            not self.red_apple
            or self.red_apple in self.snake
            or self.red_apple in self.green_apples
        ):
            self.red_apple = (
                random.randint(0, self.board_size - 1),
                random.randint(0, self.board_size - 1),
    ...
    def move_snake(self, direction=None):
        ...
        if result == "game_over":
            # if not self.agent.is_learning:
            print(
                f"Game Over! You hit a wall or yourself. "
                f"Your score is {len(self.snake)} "
                f"and max duration of {self.max_duration}"
            )
            self.is_game_over = True
            if (
                self.agent.num_training_episodes > 0
                ...
                # save q values
                print(
                    f"Training episodes left:"
                    f"{self.agent.num_training_episodes}"
                )
        elif result == "game_won":
            self.is_won = True
            # if self.agent.is_learning == False:
            print(
                f"Congratulations! You won the game with a"
                f"score of {len(self.snake)} and max duration"
                f"of {self.max_duration}"
            )
            if (
                self.agent.num_training_episodes > 0
                and self.agent.is_learning
                ...
                print(
                    f"Training episodes"
                    f"left: {self.agent.num_training_episodes}"
                )
